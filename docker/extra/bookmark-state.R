...
RestoreContext <- R6::R6Class("RestoreContext",
  public = list(
  ...
  private = list(
    ...
    loadStateQueryString = function(queryString) {
      ...
      loadFun <- function(stateDir) {
        ...
        inputsFile = paste0(stateDir, "/input.rds")
        if(object_exists(inputsFile, bucket = aws_bucket)) {
          tryCatch({
            inputValues <- s3readRDS(inputsFile, bucket = aws_bucket)
          ...
          stop("Input values file not found.")
        }
        
        valuesFile <- paste0(stateDir, "/values.rds")
        if (object_exists(valuesFile, bucket = aws_bucket)) {
          tryCatch({
            self$values <- s3readRDS(valuesFile, aws_bucket)
          },
          error = function(e) {
